...
export default function AutoComplete({
  ...
  const handleBlur = useCallback(
    (event: React.FocusEvent<HTMLInputElement, Element>) => {
      ...
      if (!allowFreeInput) handleFilterChange(selected?.label || "");
  ...
  const handleSelectOption = useCallback(
    (selectedOption: Option) => {
      handleFilterChange(customOnSelectLabeling(selectedOption));
    ...
    <CommandPrimitive onKeyDown={handleKeyDown} className="h-full">
      <div className="h-full">
        <CommandInput
          ref={inputRef}
          value={inputValue}
          onValueChange={isLoading ? undefined : handleFilterChange}
          onBlur={(event) => {
            handleBlur(event);
            props.onBlur?.(event);
          }}
